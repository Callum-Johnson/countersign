...
A round ends the loop **without a further fix** under any of three
conditions. Each names where its findings go.
...
rules' defects; everything after was lists. Blocking at the cap is a success
path in exactly the sense of the contributor policy's §3: the executor has
correctly identified that the loop is not converging, and the alternative is
another round that looks like progress.
