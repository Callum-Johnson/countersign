...
R3.1: the Review finding-line paragraph of `templates/PR-DESCRIPTION.md`
admits two ranks, `must-fix` and `note`, where §6 gives rank three states —
the third being a finding the reviewer left unranked, which round 1's repair
made "repaired at its class as a must-fix is". So the record cannot express a
state the same template elsewhere names as owing a class line; an executor
transcribing an unranked finding must write a rank nobody gave; and writing
`note` there both narrows the obligation that finding is bound by and feeds
§6's falsifier a rank nobody set. The remedy is to name `unranked` as a third
value in that parenthetical and to say the executor transcribes the absence
rather than resolving it. **That is one word, and it adds no per-review
duty.** It is deliberately not applied: applying it would be a fourth round,
which the cap reserves to the maintainer. The cost is stated here so the
maintainer can weigh the remedy without reading the template.

**What the maintainer is asked to decide.** Order a fourth round, which would
take R3.1 and carry the two notes R3.2 and R3.3 with it; or dispose of the
...
**The trend the decision turns on.** Must-fixes ran 3, 2, 1 across the three
rounds — six in total, derived from the per-round table below and not asserted
beside it. Three of those six sat inside a previous round's fix: both of round
...
argument for one more round; that half the must-fixes are still repairs of
repairs is the argument against it.
...
finds a rule defect. On 2026-09-07 the maintainer ordered fourth rounds on two
sibling tickets, and one of those rounds then found three further defects.
That is evidence toward the falsifier, and it is recorded here so this
decision is taken with it in view rather than against a cap read as a
formality.
...
**BLOCKER (2026-09-08):** the round the maintainer ordered is complete, and
the ticket blocks a second time. After round 3 reached the cap that "When
review ends" sets in `docs/tier-review-model.md`, the maintainer ordered one
further round; that round — the repair at 3d22d3b and its independent review —
is finished, and the review recorded two must-fixes. Neither is repaired here.
...
widened the inside-the-previous-fix field to name a finding from any earlier
round, so `yes, unnamed` no longer answers the question the signal asks. A
control the maintainer did not narrow is degraded while three documents assert
it is not. The remedy is to say that only a named finding answers the signal,
its number giving the round, and that `yes, unnamed` is countable by neither
the signal nor the falsifier's second arm. It is deliberately not applied.

R5.2: the round table in `templates/PR-DESCRIPTION.md` says "`m` and `n` count
must-fixes", and a must-fix line reading `yes, unnamed` can be placed neither
in that cell nor out of it, so the executor resolves it — which is the
scope-setting by the party the obligation sits on that this whole amendment
exists to remove. "Repairs of repairs" reads that cell to decide whether a
redesign is owed, so the bound party can hold a round below "mostly". The
remedy is that the cell counts only lines naming a finding, with the unnamed
reported beside it. It is deliberately not applied.

**"Repairs of repairs" fires, and the next step is not those two patches.**
Both must-fixes and five of the seven findings sit inside R3.1's fix. The
reviewer's next step is to re-run the ten-field sweep against the form the
repair produced: the sweep was run against the form at 3282adf, before the
repair, and the reviewer judges it honest and complete for that form but not
complete as a claim about the record, because it was never re-run against the
form it produced and its own new value re-opens two of its checks. That is a
redesign of the method, which is what "When review ends" asks for when a
round is mostly repairs of repairs — not the two patches R5.1 and R5.2 name.

**The trend the decision turns on.** Must-fixes ran 3, 2, 1, 2 across the four
review rounds — eight in total, derived from the per-round table below and not
asserted beside it. Five of those eight sat inside a previous round's fix:
both of round 2's, round 3's single one, and both of round 4's. The count
stopped falling at the ordered round, and the share sitting inside a previous
fix rose from half to five of eight.

**Evidence the cap's own falsifier asks for.** "When review ends" retires the
cap when a ticket blocked at it has its maintainer order a further round that
finds a rule defect. The reviewer's verdict on the ordered round is that it was
justified: it found three rule defects beyond the instance, including R1.2
recurring, which three rounds had not. That is the evidence the falsifier asks
for, recorded here so that this decision is taken with it in view rather than
against a cap read as a formality.

**What the maintainer is asked to decide.** Order a further round, which would
take the redesign the reviewer names — the sweep re-run against the form the
repair produced — and carry the five notes with it; or dispose of the ticket
another way, routing R5.1 and R5.2 to a child ticket, accepting the record as
it stands, or answering differently. The question is the maintainer's because
the cap and §3 make it so, not because the findings are unclear.

**On the round numbering.** The record numbers the maintainer-ordered repair
round 4 and holds its independent review in row 4 of the table below; the brief
that commissioned that review numbered the round 5, so the reviewer's findings
are numbered `R5.k`. §6 bars renumbering a finding, so those numbers are kept
as the reviewer wrote them rather than reconciled by changing them. Row 4 holds
that review; there is no row 5.
...
is the sum of the review rows only: round 3's must-fix is counted once, in
round 3's own row, and the repair the maintainer ordered, which took it at
...
**BLOCKER (2026-09-08):** the round the maintainer ordered is complete, and
the ticket blocks a third time. The redesign at 0d0ecfe — the ten-field sweep
re-run against the form it produced, and re-run again until a pass found
nothing new — and its independent review are finished, and the review recorded
three must-fixes. None of the three is repaired here. Whether a further round
is ordered is the maintainer's decision and not the executor's, which is the
contributor policy's §3 and the cap's own rule.

The reviewer numbered its findings `R6.k`, because `R5.k` is taken by the
review before it and §6 bars renumbering. Row 5 of the table below is this
...
countability rule does — the must-fix lines that neither name a finding nor
read `no` — in all six places. It costs nothing; it corrects a definition. It
is deliberately not applied.

R6.2: §6 and the template give as the reason only a finding number answers
that "the other three need the round, which only the number gives". That is
false. A line naming the round — `yes, round 3` — gives the round that all
three non-arm readers need, and is countable by none, so a reviewer who knows
the round but not the finding must invent a number or lose the class signal,
the first arm and the round table. The remedy is to admit a third countable
form naming the round, uncountable by the second arm alone: one clause in each
of three documents, and no new per-review duty. It is deliberately not applied.

R6.3: the Risks / follow-ups paragraph states that "the statement under the
round table runs 10 lines against 1 at 3d22d3b" and that "The five figures are
from the commands" it names. No command the description names produces 10 or 1
— How-to-verify item 12's command, run as printed, returns 11 — and seven
numbers are called five. §6's own measured-number bullet is therefore unmet in
the paragraph that carries the maintainer's 2026-09-07 cost condition. The
remedy is to name `sed -n '/where nothing was repaired/,/^$/p'
templates/PR-DESCRIPTION.md | head -n -1 | wc -l` and to correct the count. It
is deliberately not applied.

**"Closed under itself" is not sustained.** Round 5 claimed the sweep closed
at its third pass. The reviewer ran a fourth, and it found R6.1. Pass 3 asked
the round table's new count only the two questions pass 2 had raised of it —
is a reader named, is a zero written — and never asked it the sweep's own
question, which is which states this field must hold given its readers. A pass
that re-checks the previous pass's answers is not the method run again: the
method is the question, and on the newest field it was not asked. The claim
was checked and it failed.

**The class is not narrowed, but it is not closed.** The swept population grew
from ten fields to eleven when pass 1 added the cost field, and to thirteen
when pass 2 added its two statements; nothing left it unrepaired; and the two
gaps that are not this ticket's to close — the Tier line and the cost field —
are routed with named destinations under the second condition of "When review
ends". The defect is narrower than a narrowing and it is real: the newest
members of the population were swept with a narrower question than the older
ones.

**A fifth shape of the gap in the class signal, and the sharpest yet.** R6.2
is a second-column finding on the countability rule, inside R5.1's fix, one
round after R5.1 was a first-column finding on that same rule, so the signal
fires. But that rule's previous-round repair was already the class redesign
that "Repairs of repairs" had ordered, so read literally the signal now
prescribes the step just taken, and the round it says a wrong redesign costs
buys nothing. That is a fifth distinct shape beside the four already on record.
It is recorded and not answered — no wording is proposed, and the rule is in
`docs/tier-review-model.md`, which this ticket does not amend. The fact that
cuts the other way is recorded with it: on a sibling ticket closed the same
week, EM-019-001, the same wording matched a canonical case without strain, the
signal firing, the adjustment recorded beside it, the redesign taken, and the
next round finding nothing on that rule in either column. "Repairs of repairs"
fires again as well: two of the three must-fixes and four of the five findings
sit inside the previous round's fix.

**What the reviewer verified and found sound.** Four of the five routes by
...
including the per-round-row number. The falsifier is readable and can fire.
The second-instance restraint was judged honest: the closure method would
enter as a ticket's own subject on one instance, none of the bar's three
exemptions reaches it, and it was recorded in full and is reviewable — the
reviewer's one nit being that the bar names a child ticket or Risks /
follow-ups for a first instance while this one sits in Falsification.

**The trend the decision turns on.** Must-fixes ran 3, 2, 1, 2, 3 across the
five review rounds — eleven in total, derived from the per-round table below
and not asserted beside it. Seven of the eleven sat inside a previous round's
fix: both of round 2's, round 3's single one, both of round 4's, and two of
round 5's three. The eighth of those repairs-of-repairs cannot be counted
either way: R6.3's line names no finding, so it sits neither in that seven nor
out of it, and it is the `1 unnamed` the round-5 row carries. The count rose
again at the second ordered round, from two to three.

**Evidence the cap's own falsifier asks for.** "When review ends" retires the
cap when a ticket blocked at it has its maintainer order a further round that
finds a rule defect. The reviewer's verdict on the round ordered on 2026-09-08
is that it was justified: pass 2 of the re-run sweep found two real rule
defects that three reviews and one ordered round had not, and neither was
reachable without re-running the method on its own output. That is the
evidence the falsifier asks for, recorded here so that this decision is taken
with it in view rather than against a cap read as a formality.

**What the maintainer is asked to decide.** Order a further round, which would
take the three must-fixes and carry the two notes with it; or dispose of the
ticket another way — route the three to a child ticket and close on the record
as it stands, accept the record unchanged, or answer differently. The shape of
the class signal recorded above is a question about a rule this ticket does not
amend, and it is recorded rather than answered for that reason.
...
review rounds, of which 2 + 1 + 2 + 2 = seven sat inside a previous round's
fix, with one further must-fix line naming no finding and so sitting neither
in that count nor out of it. The total is the sum of the review rows only:
each repair round the maintainer ordered fills the last column of the row
whose findings it takes and adds neither a row nor a count of its own, since
a row is one independent review round. Round 5's `—` in the last column is
nothing repaired, which is the point of the block.
...
### Tier
critical in the frontmatter — process surface under the operative test: a rule
every executor works under changes meaning, and ADR-0002 governed the review
the five recorded rounds were run under. ADR-0004, from 2026-09-10, since
fixes the tier of a change to a process document at `standard` on one
independent pass; the frontmatter is left as it stands because lowering a tier
is not the executor's to do, and AC4 below records what that means for the
close.
...
silence, which the executor transcribes rather than resolves. The review of
that round found the sweep behind the repair had never been run against the
form it produced; the maintainer ordered a further round on 2026-09-08, and
round 5 re-runs the sweep until a pass finds nothing new, which took three
passes.
...
- [x] AC1: `docs/ai-contributor-policy.md` §6 states which findings the
  ...
  while every other rule that reads that field asks about the round before;
  the falsifier's own read-from clause is brought with it; and **an absence is
  ...
  carrying a value the reviewer never gave. Since round 5 §6 does not list the
  rules that read that field at all but points at the one list of them the
  template carries, and it states the line's countability on the countable
  side: only a line naming a finding, or one reading `no`, is countable, and
  any other value the reviewer wrote is reported rather than resolved.
  Since round 6 that countability sentence admits a third form — a line
  naming the round the earlier fix sits in, `yes, round 3`, countable by the
  three readers that need only the round and not by the second arm alone —
  and it is the one site that lists the forms, with every rule and count that
  reads the field naming it rather than restating the list.
  3258933 is the amendment, 8e87a58 the round-1 repair, f8880b5 the round-2
  repair, 3d22d3b the round-3 repair, 0d0ecfe the round-5 repair and 858722e
  the round-6 repair.
- [x] AC2: `docs/adr/0005-the-class-obligation-binds-a-repaired-must-fix.md`
  ...
  the figure below is unchanged and was re-read after 3d22d3b. Since round 5's
  repair it records the four rules that read the inside-the-previous-fix field
  as listed once in the template rather than a second time here, the rule that
  only a line naming a finding or reading `no` is countable by any of them,
  and the standing count of the lines no reader can count beside the second
  arm's own zero — both zero over zero rounds recorded under this rule, with
  the 1 of 5 historical lines that would have been countable stated beside
  them. That round did change the cost the maintainer accepted, and the record
  states the change: a finding line costs what it cost, and each round row now
  carries one further number. Status is `proposed`; the closing commit moves
  it to `accepted`, as ADR-0003 was.
- [x] AC3: over the 34 review rounds recorded in this repository's closed
  ...
  round-2 repair, at 3d22d3b, after the round-3 repair, and at 0d0ecfe, after
  ...
  to move.
- [x] AC4: five independent review rounds are complete, the ticket blocked at
  the cap and blocked again after the round that followed it, and the
  maintainer ordered a further round on each occasion, both on 2026-09-08. The
  ...
  3282adf returned 1 must-fix over 3 findings, inside round 2's fix, repaired
  at 3d22d3b in the first round the maintainer ordered; the review of 3d22d3b
  returned 2 must-fixes over 7 findings, both inside that repair, repaired at
  0d0ecfe in the second. The class signal in "What a review reports" fired on
  R2.2, and the repair is a redesign against its class rather than the
  adjustment the reviewer recorded beside the signal. "Repairs of repairs" in
  "When review ends" fired on the fourth review, and its repair is the
  redesign that signal asks for — the sweep re-run against the form it
  produced — and not the two patches its must-fixes name. All rounds are in
  Review below. The cap is what ended round 3 — R3.1 lay inside no limit this
  ticket records, so no condition of "When review ends" ended the loop — and
  the maintainer's order is what reopened it, which is the cap's own falsifier
  being tested rather than the cap being ignored. This criterion stays
  unticked because round 5 has not been reviewed. The branch is left checked
  out.
...
**Why this criterion is ticked without a sixth review.** The review of 0d0ecfe
returned 3 must-fixes over 5 findings, and all three are repaired at 858722e —
R6.1 and R6.2 at their class across the three documents, R6.3 with its
siblings re-read at the commits their own sentences name. What ADR-0002 asks
of this criterion has been done five times over. ADR-0004, which landed on
2026-09-10 while this ticket was blocked, since decides the tier of a change
to a process document: `standard`, one independent review pass, closing on
that pass, with the round cap and the per-round loop not applying because
there is no loop. This ticket amends `docs/ai-contributor-policy.md` and
`templates/PR-DESCRIPTION.md`, both process documents. The frontmatter still
reads `critical` and is left alone, because lowering a tier is not the
executor's to do; the maintainer's decision of 2026-09-15 is that the ticket
closes on the repair of round 5's findings, which is one pass more than the
rule now in force asks and five more than it asks of the rounds already
recorded.
...
- R3.1 — class: **which states of a finding does the record's form fail to
  ...
  names. **That scope is one line too wide, which is R5.4**: the cost field is
  conditional and not free, and round 5's re-run sweeps it. Siblings, each
  checked against `templates/PR-DESCRIPTION.md` at 3282adf:
  ...
  - **The silence of the same field**, read by the class signal in "What a
    ...
    denial. **The last clause of that repair is false, which is R5.1**: the
    signal asks about the previous round, and `yes` without a number does not
    say which round. Round 5's re-run of this sweep is below.
  ...
  - **The `Inside previous round's fix` and `Repaired by` cells of the round
    ...
    the second turning into the first when a later round repairs it. **The
    first cell's check does not survive the value this same sweep added, which
    is R5.2**: a must-fix line reading `yes, unnamed` sits neither in it nor
    out of it. Round 5's re-run is below.
...
closed tickets at 60f39fb. What grew was the template: at 3d22d3b the
finding-line statement ran 29 lines where it had run 10 at f8880b5, from `sed
-n '/^Findings, per round/,/^- R1[.]1 /p' templates/PR-DESCRIPTION.md | head
-n -1 | wc -l` and the same command on `git show
f8880b5:templates/PR-DESCRIPTION.md`; and the owed-lines statement ran 27
where it had run 23, from `sed -n '/^For each repair a line here/,/costs a
...
**Round 5's must-fixes, in §6's form.** Two must-fixes, one class, and the
class is the method behind them rather than either patch. "Repairs of repairs"
in `docs/tier-review-model.md` fired — both must-fixes and five of the seven
findings sit inside R3.1's fix, which is most of the round — and it asks for a
redesign of that fix against its class rather than a further per-finding
repair. R3.1's fix was the sweep above. There is no suite, so each sibling
says what a reader does differently.

- R5.1 and R5.2 — class: **which states of a finding does the record's form
  fail to express, given the rules that read it — asked again of the form each
  answer produces, until a pass finds nothing new?** The round-4 sweep asked
  the first half and stopped. It was honest and complete for the form it was
  run against, at 3282adf, and incomplete as a claim about the record, because
  the values it added were never put back through it: a method that changes
  what it measures is applied again to its own result. That is the redesign,
  and it is not the two patches R5.1 and R5.2 name; those are two of what the
  first re-run finds. The siblings are the passes, and each pass is listed with
  what it found. Three passes, the third finding nothing new; the documents
  carry the third pass's text.

**Pass 1, against the form at 3d22d3b.** The ten checks re-run and the sweep's
own scope re-asked. Six findings, and they are the six the review recorded as
R5.1 to R5.6.

- **The silence of the inside-the-previous-fix field, read by the class
  ...
  to answer it.
- **The round table's `Inside previous round's fix` cell, read by "The
  record." and "Repairs of repairs".** Checked "no gap" at 3282adf and
  re-opened by the same value: a must-fix line reading `yes, unnamed` sits
  neither in `m` nor out of it, so the executor placed it — the instance's own
  defect, one field further out, on the cell "Repairs of repairs" reads to
  decide whether a redesign is owed. That is R5.2. A reader of that signal now
  sees the lines the cell cannot place, rather than a proportion the bound
  party settled.
- **The list of the rules that read the field.** §6 named the second arm, the
  first arm and the class signal; the template named the falsifier, the signal
  and the round table. Two lists of one piece of reasoning, divergent in the
  round that wrote them, and neither complete: four rules read the field. That
  is R5.3. A reader now finds one list, in the template, named on its face as
  the only one.
- **The sweep's own scope.** It excluded free prose on the ground that prose
  "cannot fail to hold a state", which does not reach the cost field: that
  field is conditional rather than free, and an absence in it conflates "the
  remedy tightens nothing" with "the cost was not stated", which "A tightening
  states its cost" and its falsifier both read. That is R5.4. The scope is
  corrected here and the field is in the sweep from this pass on; the gap
  itself is recorded and routed, which is the remedy the reviewer gave, since
  the rule that owns it is in `docs/tier-review-model.md` and this ticket does
  not amend that document. Swept with it and found no gap: the repair line's
  coverage clause and the Falsification claim line's non-discriminating form,
  each of which names itself, so an absence is not ambiguous; and the
  acceptance criterion's deferral rationale, whose absence is a defect in a
  description rather than a state the form cannot hold.
- **The field's value set against the record as it stands.** `partly` is one of
  the four values a reviewer has actually written into that field, and the
...
- Re-opened by `unranked`, the other value the previous pass added, and closed:
  **the per-round must-fix count and the `m of n` cell**. An unranked finding
  is repaired at its class as a must-fix is and is not a must-fix, so it is in
  neither `m` nor `n`. Checked, no gap — the cap, the first condition of "When
  review ends" and "Repairs of repairs" are all stated on must-fixes, and §6
  reaches the class obligation and not the count, which round 4 settled.
- **The rank, the column, the repair line's three forms, the Falsification
  claim line, the acceptance-criterion tick and the Tier line** — re-checked
  against the form at 3d22d3b and unchanged by that repair, except the rank,
  whose third value is that repair itself and which no rule re-opens. The Tier
  line is still a gap and still EM-007-002's, routed and not repaired.

**Pass 2, against the form pass 1 produced.** Two findings, neither of them in
the review.

- **Pass 1's countability statement was made value by value.** It said of `yes,
  unnamed` that no reader can count it. A reviewer writes what a reviewer
  writes — `partly` is already in the record, which is R5.5 — and a statement
  that enumerates the uncountable values re-opens on the next value a reviewer
  invents, which is the same defect one level up. So the statement is made on
  the countable side and closed: a line naming a finding, or reading `no`, is
  countable; every other value is not, is reported, and is left as the reviewer
  wrote it. That is what the documents carry, and it closes R5.5 as well as
  R5.1. A reader meeting a value nobody anticipated knows what to do with it
  without asking the executor.
- **The count pass 1 put beside the round table's cell is itself a field.** It
  has a reader — "Repairs of repairs", which reads that cell to decide whether
  a redesign is owed — and pass 1 said only that the unnamed are reported
  beside it, so a round with none unnamed could not be told from a round whose
  executor did not report. `k` is therefore written even when it is zero, for
  the reason the falsification gate writes a zero red count as a zero. A reader
  of a round row can tell "none unnamed" from "not reported".

**Pass 3, against the form pass 2 produced.** The eleven checks, and the two
statements pass 2 added. Nothing new: the countability rule enumerates no
values, so no value a reviewer writes re-opens it; `k` names its reader and its
zero; and the list of the field's readers is single and says on its face that
it is the only one, so a second list cannot appear without the template saying
so. Three passes is what it took, and the sweep is closed under itself at the
third.

**The closure method is not written into the documents.** "A rule needs a
second instance" in `docs/tier-review-model.md` bars a rule from entering on
one, and a sweep re-run until it closes has one instance, which is this one. It
is recorded here as the method this round used, which is where that section
says a failure met once is recorded, and a second ticket meeting it is what
would promote it.

The notes R5.3, R5.4, R5.5 and R5.6 carry no line of their own; §6 does not
reach them, and each falls out of the class above. R5.7 asks for no change to
a rule: it is the record's own numbering, reconciled in the Review section
below.

- R6.1 — class: **which statements describe the uncountable line by what it
  fails to name, rather than by the rule that says what counts?** `k`'s
  definition is the instance. The question is asked of every statement in the
  three documents that reads or restates the field's countability. Siblings,
  each checked against the tree at 858722e:
  - **The round-table paragraph of `templates/PR-DESCRIPTION.md`** — the
    instance. Repaired: `k` reports the must-fix lines the countability
    sentence leaves uncountable.
  - **ADR-0005's sentence on what the round table costs**, at its line 130.
    Same defect, same repair.
  - **ADR-0005's statement that such a line "is countable by none of that
    field's four readers"**, at line 270 — the sentence the review quotes as
    flatly false of a `no`. Same repair.
  - **ADR-0005's Consequences bullet on the line no reader can count**, at
    line 368. Not enumerated by the review. It gave `yes, unnamed` as its
    example and so read correctly by accident, while its wording carried the
    same defect; the class question found it where a list of the review's four
    would not have.
  - **ADR-0005's Negative bullet on the count each round row costs**, at line
    379. Same defect, same repair.
  - **ADR-0005's Migration sentence on what the template carries**, at line
    432. Same defect, same repair.
  - **How-to-verify item 12 of this description.** Same defect, same repair.
  The repair is not the one the review named. Restating "neither names a
  finding nor reads `no`" in seven places would leave seven sites to update
  and six of them wrong again the moment R6.2 admits a third countable form in
  the same round. Instead the countability sentence is made the one site that
  lists the forms, and the seven statements name it — which is what "A number
  determined elsewhere has one site that goes red" asks for a count, applied
  to the definition the count reads. Where the change has no suite, what a
  reader does differently: a reader asking which lines `k` counts is sent to
  one sentence, and a reader admitting a further countable form changes one
  place rather than seven.
- R6.2 — class: **which statements give, as the reason for admitting only a
  finding number, a claim that is false of a line naming the round?** The
  sentence the review quotes is the instance, and the same sentence stands in
  three documents. Siblings, all three repaired in the one commit the rule
  this ticket adds requires: `docs/ai-contributor-policy.md` §6,
  `templates/PR-DESCRIPTION.md`, and ADR-0005's quoted replacement block.
  Checked at 858722e: `grep -rn "only the number gives" docs templates`
  returns nothing. What a reader does differently: a reviewer who knows the
  round but not the finding writes `yes, round 3`, and the three readers that
  need only the round count it, instead of inventing a number or leaving a
  line the class signal, the first arm and the round table cannot read.
- R6.3 — class: **which figures in this description does no command it names
  produce?** The round-table pair is the instance. The question is asked of
  every figure in the Risks paragraph the review quotes, each re-read at the
  commit its own sentence names. Siblings, all checked and all sound: the
  finding-line statement's 39, 29 and 10, from `sed -n '/^Findings, per
  round/,/^- R1[.]1 /p' templates/PR-DESCRIPTION.md | head -n -1 | wc -l` on
  the copies at 0d0ecfe, 3d22d3b and f8880b5; and the owed-lines statement's
  27 and 23, from `sed -n '/^For each repair a line here/,/costs a later
  reader nothing.$/p' templates/PR-DESCRIPTION.md | wc -l` on the copies at
  3d22d3b and f8880b5. Only the round-table pair had no producing command
  named: the command the description did name prints the passage rather than
  counting it, and its sed range takes the blank line that ends the passage,
  which is the one line of difference. Repaired: the paragraph names `sed -n
  '/where nothing was repaired/,/^$/p' templates/PR-DESCRIPTION.md | head -n
  -1 | wc -l`, which returns 10 and 1 on those copies. The count of figures is
  not corrected from five to seven but removed — it is a number the list in
  its own sentence determines, and §6 asks for the list rather than the
  number.

Declared, round 6: in §6 one hunk, the countability sentence and the clause
naming it as the one site. In `templates/PR-DESCRIPTION.md` two hunks, the
round table's `k` sentence and the finding-line countability prose. In
ADR-0005 seven hunks, being the six statements listed under R6.1 that name
the countability sentence, the quoted replacement block re-taken from §6 as it
now reads, and the title line the renumber changed. The three hunk counts are
from `git diff 858722e^ 858722e -M -- <path> | grep -c "^@@"`, run after
858722e. The two "as it stood" blocks in ADR-0005 are untouched. No other
paragraph of the three files changed, and nothing under `docs/tickets/` was
re-flowed by that commit.
...
is which values of the record's field the documents admit can answer it. The
gap round 5's sweep found in the cost field belongs to a rule in that
document and is routed rather than repaired, for the same reason.

Declared, round 5: in §6 the first changed line is the one beginning "arm
counts a finding in any later round", and everything above it is byte
...
signal reads; the Decision paragraph on what the two fields carry; the cost
sentence of the paragraph on the template change; that replacement block; a
new paragraph beside the standing count; two Consequences bullets; and one
sentence added to Migration. In `templates/PR-DESCRIPTION.md` three hunks: the
round table's second exemplar row with the sentence under it, the finding-line
prose, and the two finding exemplars' last field. The three hunk counts are
from `git diff 3d22d3b 0d0ecfe -- <file> | grep -c "^@@"`, run after 0d0ecfe.
No other paragraph of any of the three files changed, and nothing under
`docs/tickets/` was re-flowed.
...
10. `grep -c "sits inside the repair of a finding from an earlier"
    docs/ai-contributor-policy.md docs/adr/0005-*.md` returns 1 for each —
...
11. `git show 0d0ecfe` — the round-5 repair across the same three files.
    Take any value a reviewer could write in a finding line's last field —
    a finding number, `no`, `yes, unnamed`, `partly`, a sentence — and ask
    which of the four rules that read the field can count that line. Then
    read the template's finding-line prose and check the answer is there
    without the value having to be named in it.
12. `sed -n '/where nothing was repaired/,/^$/p' templates/PR-DESCRIPTION.md`
    — the round table's cell, which counts only must-fix lines naming a
    finding from the round before and reports beside them those the
    countability sentence leaves uncountable, with a zero written as a zero.
    The passage is printed, not counted; the count is the command named in
    Risks / follow-ups, which drops the blank line this range ends on. Ask of a round whose reviewer named no fix
    whether "Repairs of repairs" can still see it.
13. `grep -rn "answers all\|four rules" --include=*.md docs templates` — one
    list of the rules that read the inside-the-previous-fix field, in the
    template, and two pointers at it. A second list appearing is what R5.3
    was.
14. `git show 858722e` — the round-6 repair across the same three files, and
    the ADR's renumber. `grep -rn "name no finding\|names no finding\|naming
    no finding" --include=*.md docs templates` returns nothing outside this
    ticket's own quotations of the finding: the definition now lives in one
    sentence, which `grep -rln "A line is countable where it" --include=*.md
    docs templates | grep -v docs/tickets/` finds in exactly the three
    documents. Without the last filter the same grep also returns this ticket,
    which quotes the sentence in R6.1's repair line.
...
ADR-0005 was `proposed`. The commit that closes this ticket moves it to
`accepted`, which is the practice ADR-0003 established. It was renumbered
from 0004 at 858722e: ADR-0004 was taken by EM-007-002 on 2026-09-10 while
this ticket was blocked, and main's record is the published one.
...
The template grew where the finding line did not. At 0d0ecfe the finding-line
statement runs 39 lines, against 29 at 3d22d3b and 10 at f8880b5, and the
statement under the round table runs 10 lines against 1 at 3d22d3b; the
owed-lines statement is unchanged at 27, against 23 at f8880b5. The figures
above are from the two commands named in R6.3's repair line under
Falsification, each run against the named commit's copy after 0d0ecfe, and
the round-table pair is from `sed -n '/where nothing was repaired/,/^$/p'
templates/PR-DESCRIPTION.md | head -n -1 | wc -l`. How-to-verify item 12
prints that passage rather than counting it and returns one line more, being
the blank line its range ends on. Round 6 moves two of the figures: at
858722e the finding-line statement runs 43 and the statement under the round
table runs 11, from the same two commands run after that commit, and the
owed-lines statement is unchanged at 27. That is paid once by
every reader of the template and buys a record whose fields hold every state
the rules that read them name; the alternative priced against it is the
executor resolving the reviewer's silence, which is the scope-setting the
whole amendment exists to take away from the executor.

R5.4 is recorded and routed, not repaired. The cost field on a finding line is
conditional, and an absence in it conflates "the remedy tightens nothing" with
"the cost was not stated"; "A tightening states its cost" in
`docs/tier-review-model.md` and its falsifier both read that field, and this
ticket does not amend that document. It goes to whichever ticket next touches
that rule, the way R2.5 went to the limit EM-014-001's R1.9 recorded and R3.3
goes to whichever ticket adds a third asking rule. Settling here what an
...
That does not stop a reviewer answering cheaply. It stops a cheap answer
reading as evidence that the arm is quiet.

R5.7 changes no rule, and its reconciliation is in the Review section below
under "On the numbering, once". The divergence originated in the brief that
commissioned the round-4 review, not in this ticket, and it is stated as a
fact about the record and attributed to no party.

No child ticket was raised: nothing outside the amendment and the repairs of
its own rounds was found needing work. The Tier line's gap, named in round 4's
class above, is EM-007-002's, which is open, and the cost field's gap, named
in round 5's pass 1, is recorded above for whichever ticket next touches "A
tightening states its cost", which is the remedy its reviewer gave.
...
| 3 | 1 (of 3 findings) | documents: the template's Review finding-line paragraph admits two ranks where §6 gives rank three states, so the record cannot express the unranked finding the same template names as owing a class line (must-fix); §6's clause that naming several findings in one repair line is not a merge was not carried into the template; nothing makes an asking rule discoverable, so the template's inventory of the rules that ask can go stale silently | 1 of 1, 0 unnamed | 3d22d3b |
| 4 | 2 (of 7 findings) | documents: §6, ADR-0005's quoted block and the template all say a line reading `yes, unnamed` still answers the class signal, while the same round widened the field to any earlier round, so the value no longer answers the question the signal asks (must-fix); the round table's `m` and `n` count must-fixes and a must-fix line reading `yes, unnamed` sits neither in that cell nor out of it, so the executor resolves it and "Repairs of repairs" reads a cell the bound party can hold below "mostly" (must-fix); two divergent lists of the three rules one field answers; the sweep's exclusion of free prose not reaching the conditional cost field; `partly` still inadmissible, the repair covering three of four historical values; `yes, unnamed` the cheap answer on a field 1 of 5 historical lines filled admissibly; the record and the commissioning brief numbering this round differently | 2 of 2, 0 unnamed | 0d0ecfe |
| 5 | 3 (of 5 findings) | documents: `k` is defined as the must-fix lines that name no finding while the same round's rule makes `no` countable, so `k` swallows every `no` and a round answered countably throughout reports `k = n` (must-fix); the reason given for admitting only a finding number — that the other three readers need the round, which only the number gives — is false, since a line naming the round gives it and is countable by none (must-fix); the Risks paragraph states 10 and 1 from no command the description names, the command it names returning 11, and calls seven numbers five (must-fix); the numbering reconciliation states the row and round offset without stopping it recurring; §6's claim quantified over a list of readers held elsewhere | 2 of 3, 1 unnamed | 858722e |

Derived from the rows above and not asserted beside them: 3 + 2 + 1 + 2 + 3 =
**eleven** must-fixes over the five review rounds, of which none in round 1,
2 of 2 in round 2, 1 of 1 in round 3, 2 of 2 in round 4 and 2 of 3 in round 5
— **seven** — sat inside a previous round's fix. One must-fix line names no
finding: R6.3's, which is round 5's `1 unnamed`, and it sits neither in that
seven nor out of it. Every earlier round's `unnamed` count is zero, since each
of the eight must-fixes before round 5 names the finding whose repair it sits
inside or sits inside none. The total is the sum of the review rows only.
Round 3's must-fix and its `1 of 1` are counted once, in round 3's own row,
and the repair the maintainer ordered, which took that must-fix at 3d22d3b,
adds neither a row nor a count of its own: a row is one independent review
round, and the commit it produced appears in round 3's last column rather than
as a round. Round 5, the second round the maintainer ordered, stands in the
same relation to row 4: it takes that row's findings at 0d0ecfe, which is why
round 4's last column no longer reads `—`, and it adds no row of its own. Row
5 is the review of that repair, and its last column now names 858722e, the
commit that took its three must-fixes. No row 6 follows it: under ADR-0004,
which landed on 2026-09-10 while this ticket was blocked, a change to a
process document takes one independent review pass and closes on it, and the
round cap and the per-round loop do not apply. The five rows this record
carries were written under the reading ADR-0004 replaced; the maintainer's
decision of 2026-09-15 is that the ticket closes on the repair of round 5's
findings rather than on a sixth review, and that is what this row's last
column records.

No condition of "When review ends" was satisfied in any of the four review
rounds, and round 3 was the cap: the ticket blocked to the maintainer on
2026-09-08 with this record attached, and the blocker is above, immediately
before `## PR Description`, carrying the per-round table appended under it as
"When review ends" requires, since the pull-request description that table
would otherwise live in is not written until close. The maintainer read that
record and ordered a fourth round on 2026-09-08. Round 4 takes R3.1 at its
class and carries R3.2 and R3.3 with it, at 3d22d3b, which is why round 3's
last column no longer reads `—`. That round has been reviewed, its findings
are below, and no condition of "When review ends" was met by it either: the
ticket blocks again, and the second blocker of 2026-09-08 is above. The
maintainer read that second block and ordered a further round on the same
date. Round 5 takes the two must-fixes at their class — the redesign "Repairs
of repairs" asks for, and not the two patches they name — and carries R5.3 to
R5.6 with it, at 0d0ecfe, which is why round 4's last column no longer reads
`—`. R5.7 asks for no change to a rule and is reconciled below. That round has
been reviewed in turn, its findings are below as R6.1 to R6.5, and no condition
of "When review ends" was met by it either: the ticket blocks a third time, and
the third blocker of 2026-09-08 is above.
...
Deriving the owed set from §6 and the class signal alone reaches exactly what
the template implies; a note's repair owes nothing because no rule asks; a
fourth rule would be absorbed by asking rather than needing a scope written by
hand; and §6's saving clause is general rather than hand-scoped. The class
signal survives the redesign: the column, the rule and the inside-previous-fix
field still carry it, and the template demands the line "whatever the two
findings' ranks", so the case where both findings are notes still works. R2.1
is properly taken — both finding-line exemplars now carry `<rank>` and the
full placeholders. Five figures this description states were reproduced, each
by its own command:

- `34 40 180`, and the mean `5.29412` that rounds to the 5.29 stated above,
  from AC3's command with `f/n` printed as well:

      grep -rh "^| [0-9] | [0-9]* (of [0-9]" docs/tickets/done/*.md \
        | sed -E 's/^\| [0-9]+ \| ([0-9]+) \(of ([0-9]+).*/\1 \2/' \
        | awk '{m+=$1; f+=$2; n++} END {print n, m, f, f/n}'

- the 11 lines the replaced sentence ran, from `git show
  24d81a8:templates/PR-DESCRIPTION.md | sed -n '/^For each review must-fix
  repaired/,/^claim.$/p' | wc -l`.
- the 23 lines the statement that replaced it runs now, from `sed -n '/^For
  each repair a line here/,/costs a later reader nothing.$/p'
  templates/PR-DESCRIPTION.md | wc -l`.
- the 46 repair lines in `docs/tickets/done/`, of which 3 answer for several
  findings at once and each names every number, from `grep -rh "^- R[0-9]"
  docs/tickets/done/*.md | wc -l` and the same grep piped to `grep -cvE "^-
  R[0-9]+\.[0-9]+ "`.

- R3.1 · permits · must-fix · `templates/PR-DESCRIPTION.md`, the Review
  finding-line paragraph, "with the rank the reviewer gave it (`must-fix` or
  `note` …)" · the field admits two values where §6 gives rank three states —
  must-fix, note, and left unranked, which "is repaired at its class as a
  must-fix is". So the record cannot say a finding was unranked: the executor
  transcribing one must write a value the reviewer never gave, and `note`
  narrows the obligation that finding is bound by and feeds §6's falsifier's
...
silence into scope and round 2's repair closed the value set; and it adds no
entry to an enumeration maintained against an adversary. Round 3 of a cap of
three, so the ticket blocks to the maintainer with this record attached —
which "When review ends" states is a success path in exactly the sense of the
contributor policy's §3, the executor having identified that the loop is not
converging rather than taking another round that looks like progress. Nothing
in round 3 was repaired, and the table's last column says so.
...
**The round-4 review, whose findings are numbered `R5.k`.** The record numbers
the maintainer-ordered repair round 4 and reserved row 4 of the table above for
that repair's independent review; the brief that commissioned the review
numbered the round 5, so the reviewer's finding numbers read `R5.k`. §6 bars
renumbering a finding, so the numbers stand as the reviewer wrote them and are
not reconciled by changing them. Row 4 of the table is that review — the review
of the repair at 3d22d3b — and there is no row 5. R5.7 below is the reviewer's
own record of the divergence; it is stated here as a fact about the record and
attributed to no party.

**The round-4 review's independent verification, recorded before its
findings.** The reviewer tested the ten-field sweep rather than accepting it,
and reports it honest and complete **for the form as it stood at 3282adf**: it
enumerates the Tier line, the acceptance-criterion tick, the Falsification
claim line, the repair line, the round number, the three table cells, the
column, the rank and the inside-the-previous-fix field; it misses no
closed-value field that a rule reads; and its five "no gap" checks are sound.
The reviewer also confirms that R1.2's recurrence is real: at f8880b5 the field
was owed where a finding sits inside the previous round's fix, while the
...
- the finding-line statement at 29 lines against 10, from `sed -n
  ...
  -1 | wc -l` and the same command on `git show
  f8880b5:templates/PR-DESCRIPTION.md`.
- the owed-lines statement at 27 lines against 23, from `sed -n '/^For each
  repair a line here/,/costs a later reader nothing.$/p'
  templates/PR-DESCRIPTION.md | wc -l` and the same on that commit's copy.
- the 5 inside-the-previous-fix lines in `docs/tickets/done/`, of which 1 is
  admissible under the form as it stood — one `no`, three `yes` forms and one
  `partly` — from `grep -rh "inside previous fix" docs/tickets/done/*.md | wc
  -l` and the same grep piped to `grep -cE "inside previous fix:
  (no|R[0-9]+[.][0-9]+)"`, with `grep -c` returning 1 for each of the values.
- the 46 repair lines in `docs/tickets/done/`, of which 3 answer for several
...
- R5.1 · permits · must-fix · §6, ADR-0005's quoted block and
  ...
  any earlier round, so `yes, unnamed` no longer answers it — a control the
  maintainer did not narrow is degraded while three documents assert it is not
  — remedy: only a named finding answers the signal, its number giving the
  round, and `yes, unnamed` is countable by neither the signal nor the
  falsifier's second arm; inside previous fix: R3.1. **Taken at 0d0ecfe, at
  its class rather than at the three sentences.** It sits inside R3.1's fix and
  "Repairs of repairs" fired, so the repair is the sweep re-run against the
  form it produced; the remedy the reviewer named is applied and then
  generalised by the second pass, from `yes, unnamed` to every value a
  reviewer can write. The class, its passes and its siblings are under
  Falsification above.
- R5.2 · permits · must-fix · `templates/PR-DESCRIPTION.md`, the round table,
  "`m` and `n` count must-fixes" · a must-fix line reading `yes, unnamed` can
  be placed neither in that cell nor out of it, so the executor resolves it,
  and "Repairs of repairs" reads the cell to decide whether a redesign is
  owed, so the party the control sits on can hold a round below "mostly" —
  remedy: the cell counts only lines naming a finding, with the unnamed
  reported beside it; inside previous fix: R3.1. **Taken at 0d0ecfe** as a
  sibling of the same class: the cell counts only must-fix lines naming a
  finding from the round before, and reports beside them the count of must-fix
  lines naming none, written even when it is zero.
- R5.3 · permits · note · the "one field answers all three" sentence · §6's
  three are the second arm, the first arm and the class signal; the template's
  three are the falsifier, the signal and the round table — two lists of one
...
- R5.4 · permits · note · the sweep's exclusion, "a field of free prose …
  ...
  conditional rather than free: absence there conflates "does not tighten"
  with "cost unstated", and both "A tightening states its cost" and its
  falsifier read it — remedy: recorded for the next ticket touching that rule;
  inside previous fix: R3.1. **Taken at 0d0ecfe as the scope correction
  only.** The sweep now reaches conditional fields as well as closed-value
  ones, and swept the cost field; what it found there is recorded and routed
  in Risks / follow-ups above, to whichever ticket next touches "A tightening
  states its cost", which is the remedy the reviewer gave.
- R5.5 · permits · note · the silence sibling · `partly`, one of the four
  historical values the sweep itself quotes, is still inadmissible, and the
  description does not say the repair covers three of the four — remedy: say
  so; inside previous fix: R3.1. **Taken at 0d0ecfe**: the second pass
  replaced the per-value statement with one made on the countable side, so
  `partly` and any other value a reviewer writes are covered without being
  named, and the description says which of the four historical values the
...
- R5.6 · permits · note · §6, "the cost of the silence" · `yes, unnamed` is the
  ...
  them.
- R5.7 · permits · note · Review section numbering · the record calls the
  repair round 4 and reserves row 4 for its review, which the brief that
  commissioned that review numbered 5; §6 bars renumbering, so the `R5.k`
  finding numbers must be reconciled in the record — remedy: state the
  divergence and which row holds which; inside previous fix: —. **Taken at
  0d0ecfe as a reconciliation of the record and no change to a rule**, under
  "On the numbering, once" below.

R5.7's last field reads `—` because that is what the reviewer wrote where the
form gives `no`; an absence is transcribed as an absence, and the executor does
not resolve it into a value the reviewer did not give. Under the rule round 5
writes, it is one of the values no reader of the field can count, which the
line now says on its face; R5.7 is a note, so the round table's count of
unnamed must-fix lines does not reach it in either direction.

No condition of "When review ends" was met by the round-4 review. It found two
must-fixes; neither lies inside a limit this ticket records, since both are new
and were created by the round-4 repair itself; and neither adds an entry to an
enumeration maintained against an adversary. The class signal did not fire:
every finding is in the first column, so no rule carries a second-column
finding inside the fix of a first-column one.

**"Repairs of repairs" fires.** Both must-fixes and five of the seven findings
sit inside R3.1's fix, which is most of the round, and "When review ends" says
that the next step is then a redesign of the fix against its class rather than
a further per-finding repair. The reviewer's stated next step is to re-run the
ten-field sweep against the form the repair produced, not to take two patches.
Its verdict on the sweep is the crux: honest and complete for the form at
3282adf, but not complete as a claim about the record, because it was never
re-run against the form it produced, and its own new value re-opens two of its
checks.

**The reviewer's verdict on the ordered round: justified.** It found three rule
defects beyond the instance, R1.2's recurrence among them, which three rounds
had not.

Post-review tree check after the round-4 review and before the first edit of
this record: `git status --porcelain` empty, and `git worktree list` showing
only worktrees this ticket's executor did not create.

Nothing found in the round-4 review was repaired at the block: the ticket
blocked to the maintainer a second time, and the second blocker of 2026-09-08
is above.

**Round 5, the second round the maintainer ordered.** The maintainer read that
record and ordered a further round on 2026-09-08. "Repairs of repairs" had
fired — both must-fixes and five of the seven findings sat inside R3.1's fix —
and what it asks for is a redesign of that fix against its class rather than a
further per-finding repair. The fix was the ten-field sweep; the redesign is
the sweep re-run against the form it produced, and re-run again until a pass
finds nothing new, since a method that changes what it measures is applied
again to its own result. Three passes. Pass 1, against the form at 3d22d3b,
found six, and they are the six the review recorded as R5.1 to R5.6. Pass 2,
against the form pass 1 produced, found two the review had not: a countability
statement made value by value re-opens on the next value a reviewer writes,
and the count pass 1 put beside the round table's cell had no reader named and
no zero convention. Pass 3 found nothing new. The class, the passes and each
sibling are under Falsification above, and the documents carry the third
pass's text.

Nothing in round 5 is a review finding, since round 5 is a repair round and
not a review; row 5 waits on the independent review of this head and is not
written until that review exists. What round 5 says about the conditions of
"When review ends" is what the round-4 review already said — neither must-fix
lay inside a limit this ticket records, since both were created by the round-4
repair itself, and neither added an entry to an enumeration maintained against
an adversary — and the loop was ended by the cap and reopened by the
maintainer, not by a condition.

Post-review tree check taken again after the maintainer's order and before the
first edit of round 5, since the tree had been left standing between: `git
status --porcelain` empty, and `git worktree list` showing only worktrees this
ticket's executor did not create — the main checkout, this ticket's own
worktree, and the ticket and review worktrees of other tickets, none of them
made here. Two of those other worktrees moved between the two checks, which is
sibling tickets being worked and not this tree being written to.

**On the numbering, once.** A row is one independent review round. Row 1 holds
the review of a586c67, row 2 the review of 24d81a8, row 3 the review of
3282adf and row 4 the review of 3d22d3b; a repair round's commit fills the last
column of the row whose findings it takes, so 3d22d3b fills row 3's and
0d0ecfe fills row 4's. The brief that commissioned row 4's review numbered that
round 5, which is why its findings read `R5.k` while this record calls the
repair preceding them round 4. §6 bars renumbering, so the numbers stand as the
reviewer wrote them. So "round 5" names two things across the surrounding
material — the review held in row 4, as its commissioning brief numbered it,
and the repair round that takes that review's findings, as this record numbers
it; this record means the second, and calls the first row 4. The divergence
originated in the commissioning brief and not in this ticket, and it is stated
as a fact about the record, attributed to no party.

The offset then recurs by one row. Row 5 holds the review of 0d0ecfe, and its
reviewer numbered its findings `R6.k`: `R5.k` was taken by row 4's review, and
§6 bars renumbering, so the numbers a review can write are the numbers no
earlier review used. So each row from row 4 on carries findings numbered one
higher than the row, and the record says so once, here, rather than in each
row. R6.4 is the reviewer's own finding that this reconciliation states the
offset without stopping it recurring; it asks for no change to a rule, and the
remedy its reviewer gave is that it be recorded.

**The round-5 review's independent verification, recorded before its
findings.** The reviewer tested the redesign rather than accepting it. Of the
routes by which the executor could still set the scope of its own obligation,
...
- the finding-line statement at 39 lines, against 29 at 3d22d3b and 10 at
  f8880b5, from `sed -n '/^Findings, per round/,/^- R1[.]1 /p'
  templates/PR-DESCRIPTION.md | head -n -1 | wc -l` and the same command on
  each of those commits' copies.
- the owed-lines statement unchanged at 27, against 23 at f8880b5, from `sed
  -n '/^For each repair a line here/,/costs a later reader nothing.$/p'
  templates/PR-DESCRIPTION.md | wc -l` and the same on that commit's copy.
- the round-table statement at 10 lines against 1 at 3d22d3b — reproduced only
  after correcting the command the description names for it, which is R6.3
  below; the command as printed returns 11.
- the 5 inside-the-previous-fix lines in `docs/tickets/done/`, of which 1 is
  countable, and the 46 repair lines there, of which 3 answer for several
  findings at once, from the greps R2.3 and the round-4 review's verification
  name, over the same untouched directory.

**The second-instance restraint was judged honest.** "A rule needs a second
instance" in `docs/tier-review-model.md` bars a rule from entering as a
ticket's own subject on one instance. The closure method round 5 used — a
sweep re-run against its own output until a pass finds nothing new — would
enter on one instance, and none of the bar's three exemptions reaches it: it
is not a repair of the change under review, not an entry in a list, and not an
amendment under "Retiring a control". It was recorded in full and is
reviewable. The reviewer's one nit is that the bar names a child ticket under
§4 or Risks / follow-ups as where a first instance is recorded, while this one
sits in Falsification.
...
- R6.1 · permits · must-fix · `templates/PR-DESCRIPTION.md`'s round-table
  ...
  define `k` on the countable side, as the countability rule does — the
  must-fix lines that neither name a finding nor read `no` — in all six
  places; cost: none, it corrects a definition; inside previous fix: R5.2
- R6.2 · refuses · must-fix · §6 and `templates/PR-DESCRIPTION.md`, "the other
  three need the round, which only the number gives" · false: a line naming
  the round, `yes, round 3`, gives the round that all three non-arm readers
  need, yet is countable by none — so a reviewer who knows the round but not
  the finding must invent a number or lose the class signal, the first arm and
  the round table — remedy: admit a third countable form naming the round,
  uncountable by the second arm alone; cost: one clause in each of three
  documents, and no new per-review duty; inside previous fix: R5.1. **The
  class signal fires here**, and what it asks for is below.
- R6.3 · permits · must-fix · Risks / follow-ups, "the statement under the
  round table runs 10 lines against 1 at 3d22d3b … The five figures are from
  the commands" · 10 and 1 are produced by no command the description names —
  How-to-verify item 12's command, run as printed, returns 11 — and seven
  numbers are called five. §6's own measured-number bullet is unmet in the
  paragraph carrying the maintainer's 2026-09-07 cost condition — remedy: name
  `sed -n '/where nothing was repaired/,/^$/p' templates/PR-DESCRIPTION.md |
  head -n -1 | wc -l` and correct the count; inside previous fix: —
- R6.4 · permits · note · the Review section's numbering reconciliation, "On
  the numbering, once" · it states the row and round offset but does not stop
  it recurring; this review could not number for the row it fills without
  colliding with the numbers the review before it wrote — remedy: none that
  tightens; recorded; inside previous fix: R5.7
- R6.5 · permits · note · §6, "every other rule that reads the field asks
  about the round before" · a claim quantified over a list held elsewhere,
  which the repair moved out precisely to stop drift — remedy: none that
  tightens; recorded for whoever adds a fifth reader; inside previous fix: R5.3

R6.3's last field reads `—` because that is what the reviewer wrote where the
form gives `no`; an absence is transcribed as an absence. Under the rule round
5 wrote, it is a value no reader of the field can count, and R6.3 is a
must-fix, so it is the round-5 row's `1 unnamed`: it sits neither in that row's
`m` nor out of it, which is the count `k` exists to make visible.

No condition of "When review ends" was met by the round-5 review. It found
three must-fixes; none lies inside a limit this ticket records, since all three
were created by the round-5 repair itself; and none adds an entry to an
enumeration maintained against an adversary.

**The class signal fires, on R6.2, and this is a fifth shape of the gap in
it.** R5.1 was a first-column finding on the countability rule; R6.2 is a
second-column finding on that same rule, one round later and inside R5.1's
fix, which is the both-columns-on-one-rule signal in "What a review reports"
read from the two fields every finding line carries. What the signal asks for
is a redesign of the round's repair of that rule against its class. But that
rule's previous-round repair was already the class redesign that "Repairs of
repairs" had ordered, so read literally the signal now prescribes the step just
taken, and the round it says a wrong redesign costs buys nothing. That is a
fifth distinct shape of the gap in the signal's wording, beside the four
already on record, and it is the sharpest of them, because the two rules point
at the same step rather than at different ones. It is recorded and not
answered: no wording is proposed here, the rule is in
`docs/tier-review-model.md`, and this ticket does not amend that document.
Beside it belongs the fact that cuts the other way — on a sibling ticket closed
the same week, EM-019-001, the same wording matched a canonical case without
strain: the signal fired, the reviewer recorded beside it the adjustment it
would otherwise have proposed, the executor took the redesign, and the next
round found nothing on that rule in either column.

**"Repairs of repairs" fires again.** Two of the three must-fixes and four of
the five findings sit inside the previous round's fix, which is most of the
round, so the model asks again for a redesign against the class rather than a
further per-finding repair. What that means here is the subject of the two
paragraphs below, and it is the maintainer's to order.

**"Closed under itself" is not sustained.** Round 5 claimed the sweep closed
at its third pass. The reviewer ran a fourth pass, and it found R6.1. Pass 3
asked the round table's new count only the two questions pass 2 had raised of
it — is a reader named, is a zero written — and never asked it the sweep's own
question, which is *which states must this field hold, given its readers*. A
pass that re-checks the previous pass's answers is not the method run again;
the method is the question, and on the newest field it was not asked. The
claim was checked and it failed, and R6.1 is what the unasked question
returns.

**The class is not narrowed, but it is not closed.** The swept population grew
from ten fields to eleven when pass 1 added the cost field, and to thirteen
when pass 2 added its two statements; nothing left the population unrepaired;
and the two gaps that are not this ticket's to close — the Tier line and the
cost field — are routed with named destinations under the second condition of
"When review ends". The defect the fourth pass found is narrower than that and
it is real: the newest members of the population were swept with a narrower
question than the older ones.

**The reviewer's verdict on the ordered round: justified.** Pass 2 found two
real rule defects that three reviews and one ordered round had not, and
neither was reachable without re-running the method on its own output. That is
the evidence the cap's own falsifier asks for, and it is recorded so that the
maintainer's decision is taken with it in view.

Post-review tree check after the round-5 review and before the first edit of
this record: `git status --porcelain` empty, and `git worktree list` showing
only worktrees this ticket's executor did not create — the main checkout, this
ticket's own worktree, and the ticket and review worktrees of other tickets,
none of them made here.

Nothing found in the round-5 review was repaired at the block: the ticket
blocked to the maintainer a third time, and the third blocker of 2026-09-08 is
above. Nothing is closed, merged or pushed, and the branch is left checked out.
